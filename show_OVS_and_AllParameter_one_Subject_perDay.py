...
for var_count, variable in enumerate(allkeys):
    ...
    pdf_filename = f'{variable}_vs_OVS_EachSubjects_perDay.pdf'
    ...
    for subject_counter, onesubject in enumerate(subjects):
        ...
        data_onesubject_onestudy2 = data_onesubject.query(f"studyname=='EMA2'")
        days2 = data_onesubject_onestudy["day"].unique()

        if (len(days2) > len(days)):
            nr_of_days = len(days2)
        else:
            nr_of_days = len(days)

        for day_counter in range(nr_of_days):
            ...
            if (day_counter < len(days)):
                ...
                for chunk_counter, chunk_data in enumerate(cont_chunks_data):
                    ...
                    ax[1].plot(cont_chunks_time[chunk_counter], chunk_data,  color = newcolor[color_counter], alpha = 0.5)
            ...
            if (day_counter < len(days2)):
                one_day_data2 =  data_onesubject_onestudy2.query(f"day=={day_counter}")
            
                cont_chunks_data, cont_chunks_time, cont_chunks_ovs =  get_continous_chunk_data_and_time_and_OVS(one_day_data2,variable)

                for chunk_counter, chunk_data in enumerate(cont_chunks_data):
                    ax[2].plot(cont_chunks_time[chunk_counter], cont_chunks_ovs[chunk_counter],  color = newcolor[color_counter], alpha = 0.5)
                    ax[3].plot(cont_chunks_time[chunk_counter], chunk_data,  color = newcolor[color_counter], alpha = 0.5)
                ...
                titletext = f'{onesubject}_{day_counter}_OVS[%]_EMA2'
    ...
    outpdf.close()
    del global_data, cont_chunks_data, cont_chunks_time, cont_chunks_ovs, data_onesubject_onestudy, data_onesubject, outpdf, fig, ax, one_day_data
    gc.collect()
