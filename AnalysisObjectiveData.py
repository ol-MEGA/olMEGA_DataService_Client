...
# HERE: Audio Analyses                 
                ...
                resultentry.update({"meanPegel": 10*np.log10(np.mean(rms_lin))})
                resultentry.update({"varPegel": 10*np.log10(np.var(rms_lin))})
                ...
                for freqcounter, freq in enumerate(f_nominal):
                    ...
                    resultentry.update({f"meanOctav{freq}": 10*np.log10(np.mean(octavPSD_one[:,freqcounter]))})
                    resultentry.update({f"varOctav{freq}": 10*np.log10(np.var(octavPSD_one[:,freqcounter]))})
...
# only the ownvoice parts
                if (OVD_percent>0 and len(rms_psd) == len(curOVD)):
                    ...
                    resultentry.update({"meanPegel_ov": 10*np.log10(np.mean(rms_lin_ov))})
                    resultentry.update({"varPegel_ov": 10*np.log10(np.var(rms_lin_ov))})
                    ...
                    for freqcounter, freq in enumerate(f_nominal):
                        #hist_result, small_result, high_result = get_histogram(10*np.log10(octavPSD[:,freqcounter]),hist_min,hist_max)
                        resultentry.update({f"meanOctav_ov{freq}": 10*np.log10(np.mean(octavPSD_one_ov[:,freqcounter]))})
                        resultentry.update({f"varOctav_ov{freq}": 10*np.log10(np.var(octavPSD_one_ov[:,freqcounter]))})
...
# only the without ownvoice parts
                if (OVD_percent<1 and len(rms_psd) == len(curOVD)):
                    ...
                    resultentry.update({"meanPegel_notov": 10*np.log10(np.mean(rms_lin_notov))})
                    resultentry.update({"varPegel_notov": 10*np.log10(np.var(rms_lin_notov))})
                    ...
                    for freqcounter, freq in enumerate(f_nominal):
                        #hist_result, small_result, high_result = get_histogram(10*np.log10(octavPSD[:,freqcounter]),hist_min,hist_max)
                        resultentry.update({f"meanOctav_notov{freq}": 10*np.log10(np.mean(octavPSD_one_notov[:,freqcounter]))})
                        resultentry.update({f"varOctav_notov{freq}": 10*np.log10(np.var(octavPSD_one_notov[:,freqcounter]))})
